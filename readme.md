...
> Retrieval-Augmented Generation system for Tata Nexon BS6 service manual intelligence.
...
Tata Nexon BS6 SUV. These professionals and owners rely on the official
Tata Nexon BS6 Owner's Manual to perform diagnostics, scheduled maintenance,
fault identification, and component-level repairs.
Problem Description
The Tata Nexon BS6 Owner's Manual is a comprehensive, multimodal document
...
Nexon BS6 under full load, or a vehicle owner trying to understand what a
...
Unlike a generic document Q&A system, the Tata Nexon BS6 Owner's Manual
...
norms, and Kryotec diesel engine specifications are Nexon-specific and
context-dependent.
...
service interval for the Nexon's Kryotec 2.0L diesel engine can cause
...
Fine-tuning a language model on the Tata Nexon BS6 manual is impractical —
...
official Tata Nexon BS6 Owner's Manual.
Expected Outcomes
...
on the Nexon?" → returns step-by-step procedure with page reference.
...
```
5. Add sample PDF
Place your PDF in the `sample\_documents/` folder:
```
sample\_documents/Nexon-bs6-owners-manual.pdf
```
6. Run the server
```bash
...
{
  "status": "ok",
  "model\_llm": "google/gemma-3-27b-it:free",
  "model\_vlm": "nvidia/nemotron-nano-12b-v2-vl:free",
  "model\_embedding": "all-MiniLM-L6-v2",
  "chunks\_indexed": 2378,
  "uptime\_seconds": 53.07,
...
```json
{
  ...
  "chunks\_indexed": 2378,
  "total\_indexed": 2378,
  "processing\_time\_seconds": 45.2
}
```
POST /query
